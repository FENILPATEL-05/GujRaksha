...
if [ "$1" == "--source" ]; then
  ...
  echo "======================================================================"
  exec "$PY_BIN" "$SCRIPT_DIR/ai_worker.py" --source "$SOURCE" --central-url "$CENTRAL_URL" --camera-code "$CAM_CODE"
else
  ...
  echo "======================================================================"
  exec "$PY_BIN" "$SCRIPT_DIR/ai_worker.py" --central-url "$CENTRAL_URL" --worker-id "$WORKER_ID" --max-capacity "$MAX_CAPACITY"
fi
