...
class CameraDataStore {
  ...
  getAll(filters = {}) {
    ...
    const hasMode = Boolean(filters.detection_mode && filters.detection_mode !== 'ALL');
    const modeQuery = hasMode ? filters.detection_mode.toUpperCase().trim() : '';
    ...
    if (!hasBbox && !hasDept && !hasDist && !hasStatus && !hasMode && !hasOwnership && !hasSearch) {
    ...
    for (let i = 0; i < totalCandidates; i++) {
      ...
      if (hasMode && c._norm_mode !== modeQuery && !(modeQuery === 'ANPR_DETECTION' && c._norm_mode === 'ANPR')) {
        continue;
      }
  ...
  async update(id, updateData) {
    ...
    const updated = {
      ...
      detection_mode: updateData.detection_mode !== undefined ? updateData.detection_mode : (existing.detection_mode || 'GENERAL_SURVEILLANCE'),
