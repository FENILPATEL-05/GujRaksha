...
if command -v nvidia-smi &>/dev/null; then
  IS_GPU_OK=$("$VENV_DIR/bin/python" -c "import onnxruntime as ort; print(bool(hasattr(ort, 'get_available_providers') and 'CUDAExecutionProvider' in ort.get_available_providers()))" 2>/dev/null || echo "False")
  if [ "$IS_GPU_OK" == "True" ]; then
    echo "✅ GPU Acceleration already active (CUDAExecutionProvider verified)."
  else
    echo "⚡ NVIDIA GPU Detected! Clean installing GPU-accelerated ONNX Runtime..."
    "$VENV_DIR/bin/pip" uninstall -y onnxruntime onnxruntime-gpu 2>/dev/null || true
    "$VENV_DIR/bin/pip" install --force-reinstall onnxruntime-gpu
